...
from ..core .constants import APP_ID ,DEFAULT_INDEXES_FILE ,USER_INDEXES_FILE ,REGISTRY_CACHE_TTL 
...
class RegistryService :
    ...
    def get_packages (self )->List [Dict [str ,Any ]]:
        ...
        if self ._packages_cache is not None and self ._packages_cache_time is not None :
            try :
                if (time .time ()-float (self ._packages_cache_time ))<float (REGISTRY_CACHE_TTL ):
