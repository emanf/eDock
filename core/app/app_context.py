...
class AppContext:
    ...
    def run_file(self, target, cwd=None):
        ...
        try:
            ...
            if sys.platform == "darwin":
                subprocess.Popen(
                    ["open", str(target_path)], cwd=str(cwd) if cwd else None
                )
                return True

            subprocess.Popen(
                ["xdg-open", str(target_path)], cwd=str(cwd) if cwd else None
            )
            return True
    ...
    def run_command(self, command, cwd=None):
        ...
        try:
            subprocess.Popen(
                command, cwd=str(cwd) if cwd else None, shell=isinstance(command, str)
            )
            return True
        except Exception as e:
            print_and_log(
    ...
    def app_info(self, app_instance):
        ...
        return self.safe_app_call(
            ...
            "show_info",
            lambda: (
                app_instance.show_info() if hasattr(app_instance, "show_info") else None
            ),
            default=None,
        )
