...
class DockButtonMenu(QMenu):
    def __init__(self, parent_button):
        ...
        self._menu_icon_style = DockButtonMenuIconStyle(
            self.style(), self._menu_icon_size
        )
    ...
    def _create_action(self, title, icon=None, callback=None, enabled=True):
        ...
        if icon:
            qicon = self._create_material_icon(
                icon, size=self._menu_icon_size, color=self.get_menu_icon_color()
            )
    ...
    def _normalize_custom_menu(menu_data):
        ...
        if isinstance(menu_data, dict):
            if "items" in menu_data:
                normalized["hide_default_menu"] = bool(
                    menu_data.get("hide_default_menu", False)
                )
            ...
            for title, callback in menu_data.items():
                if str(title).strip().lower() == "separator":
                    ...
                    items.append(
                        {"type": "menu", "title": str(title), "action": callback}
                    )
            normalized["items"] = items
            return normalized
    ...
    def add_custom_items(self, items):
        ...
        for item in items:
            ...
            enabled = (
                item.get("enabled", True) if item_type != "disabled-menu" else False
            )

            self._create_action(
                title=title, icon=icon, callback=callback, enabled=enabled
            )
    ...
    def execute(button, pos):
        ...
        if app_instance:
            if hasattr(app_instance, "get_context_menu"):
                try:
                    context_menu_data = app_instance.get_context_menu()
                except Exception:
                    context_menu_data = None

        normalized_menu = menu._normalize_custom_menu(context_menu_data)
        ...
        if not hide_default_menu:
            ...
            act_clear_cache = menu._create_action(
                title="Clear app cache", icon="delete"
            )
            act_clear_data = menu._create_action(title="Clear app data", icon="warning")
            act_remove = menu._create_action(
                title="Remove App from Dock", icon="remove_circle"
            )
        ...
        if selected == act_info:
            ...
            if hasattr(button, "clear_app_cache"):
                if ConfirmDialog.show(
                    ...
                    message="The app cache will be cleared.",
                    icon="delete",
                    confirm_button_text="Clear Cache",
                ):
            ...
            if hasattr(button, "clear_app_data"):
                if ConfirmDialog.show(
                    ...
                    message="All saved data will be deleted.",
                    icon="warning",
                    confirm_button_text="Delete Data",
                ):
            ...
            if hasattr(button, "remove_button"):
                if ConfirmDialog.show(
                    parent=button,
                    title="Remove App",
                    message=f"Remove {app_name} from the dock?",
                    icon="remove_circle",
