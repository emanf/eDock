...
class ShortcutCaptureDialog(QDialog):
    def __init__(self, current_shortcut="", parent=None):
        ...
        try:
            ...
            loaded = None
            if parent_widget is not None and hasattr(parent_widget, "loaded_app"):
                loaded = getattr(parent_widget, "loaded_app", None)

            dock = (
                getattr(parent_widget, "dock_window", None)
            ...
            if (
                ...
                and hasattr(dock, "get_app_shortcut")
            ):
                try:
                    latest = dock.get_app_shortcut(loaded)
                    if latest:
                        self.current_shortcut = str(latest or "")
                        self.shortcut_value = self.current_shortcut
                except Exception:
                    pass
    ...
    def get_theme_colors(self):
        ...
        return {
            ...
            "normal_border_width": normal_button.get(
                Theme.Components.Button.BORDER_WIDTH
            ),
            "normal_border_radius": normal_button.get(
                Theme.Components.Button.BORDER_RADIUS
            ),
            "primary_background": uic(
                primary_button.get(Theme.Components.Button.BACKGROUND_COLOR)
            ),
            "primary_hover": uic(
                primary_button.get(Theme.Components.Button.HOVER_COLOR)
            ),
            "primary_pressed": uic(
                primary_button.get(Theme.Components.Button.PRESSED_COLOR)
            ),
            "primary_border": uic(
                primary_button.get(Theme.Components.Button.BORDER_COLOR)
            ),
            "primary_text": uic(primary_button.get(Theme.Components.Button.TEXT_COLOR)),
            "primary_border_width": primary_button.get(
                Theme.Components.Button.BORDER_WIDTH
            ),
            "primary_border_radius": primary_button.get(
                Theme.Components.Button.BORDER_RADIUS
            ),
    ...
    def save_shortcut(self):
        ...
        if seq.isEmpty():
            ...
            try:
                ...
                le = self.shortcut_edit.findChild(QLineEdit)
                if le is not None:
                    le.clear()
            except Exception:
                pass
    ...
    def load_shortcut(self, app_id=None, config_manager=None) -> str:
        shortcut = ""

        try:
            app_id = app_id or self.app_id
            config_manager = config_manager or self.config_manager

            if config_manager is None:
                widget = self.parent()
                while widget:
                    cm = getattr(widget, "config_manager", None)
                    if cm is not None:
                        config_manager = cm
                        break
                    ...
                    dock_window = getattr(widget, "dock_window", None)
                    if dock_window is not None:
                        cm = getattr(dock_window, "config_manager", None)
                        if cm is not None:
                            config_manager = cm
                            break
            ...
            if config_manager is not None and app_id:
                try:
                    shortcut = str(config_manager.get_shortcut(app_id, "") or "")
                except Exception:
                    shortcut = ""

            self.current_shortcut = shortcut
            self.shortcut_value = shortcut
