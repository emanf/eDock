...
def dock_size_for_icon_size(window, icon_size):
    ...
    if window.dock_position in ("top", "bottom"):
        width = window.inner_padding * 2 + count * (icon_size + 8) + max(0, count - 1) * window.spacing
        height = window.inner_padding * 2 + icon_size + 8
    else:
        width = window.inner_padding * 2 + icon_size + 8
        height = window.inner_padding * 2 + count * (icon_size + 8) + max(0, count - 1) * window.spacing
...
def update_effective_icon_size(window):
    ...
    natural_length = natural_width if window.dock_position in ("top", "bottom") else natural_height
    ...
    available_for_buttons = max_length - window.inner_padding * 2 - max(0, count - 1) * window.spacing
    ...
    fitted_width, fitted_height = window.dock_size_for_icon_size(window.effective_icon_size)
    fitted_length = fitted_width if window.dock_position in ("top", "bottom") else fitted_height
...
def update_dock_geometry(window):
    ...
    content_width, content_height = window.dock_size_for_icon_size(window.effective_icon_size)
    width = content_width
...
def reposition_dock(window):
    ...
    if window.dock_position == "left":
        ...
        window.hidden_pos = QPoint(
            geom.x() - window.width() + window.peek_width,
            y
        )
        ...
        window.shown_pos = QPoint(
            geom.x() + geom.width() - window.width() + edge_guard,
            y
        )

        window.hidden_pos = QPoint(
            geom.x() + geom.width() - window.peek_width,
            y
        )
        ...
        window.hidden_pos = QPoint(
            x,
            geom.y() - window.height() + window.peek_width
        )
        ...
        window.shown_pos = QPoint(
            x,
            geom.y() + geom.height() - window.height() + edge_guard
        )

        window.hidden_pos = QPoint(
            x,
            geom.y() + geom.height() - window.peek_width
        )
