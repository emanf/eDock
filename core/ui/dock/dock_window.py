...
class DockWindow(eD_Widget):
    ...
    def __init__(self, config_manager=None, app_loader=None, app=None, context=None, parent=None):
        ...
        self.runtime_service = AppRuntimeService(self.app_api if self.app_api is not None else self.context)
    ...
    def notify_apps_theme_changed(self):
        runtime_service = self.context.get('runtime_service') if isinstance(self.context, dict) else None
        if runtime_service is not None and hasattr(runtime_service, 'broadcast_theme_changed'):
            try:
    ...
    def eventFilter(self, source, event):
        ...
        if event.type() == QEvent.MouseButtonPress and event.button() == Qt.LeftButton:
            if source is self.scroll_area.viewport() or source is self.content_widget:
                ...
                self.drag_scroll_start_h = self.scroll_area.horizontalScrollBar().value()
        ...
        if event.type() == QEvent.MouseMove and self.drag_scroll_pressed:
            ...
            if not self.drag_scroll_active:
                if abs(delta.x()) >= self.drag_scroll_threshold or abs(delta.y()) >= self.drag_scroll_threshold:
        ...
        if event.type() == QEvent.MouseButtonRelease and event.button() == Qt.LeftButton:
            ...
            if was_dragging and (source is self.scroll_area.viewport() or source is self.content_widget):
                return True
    ...
    def ensure_default_config(self):
        ...
        if not config_path.exists():
            if "dock" not in default_config or not isinstance(default_config["dock"], dict):
                default_config["dock"] = {}

            default_config["dock"].setdefault("resize-mode", ResizeMode.RESIZABLE_BUTTON)
    ...
    def show_custom_tooltip(self, button):
        ...
        if self.dock_position == "left":
            ...
            x = button.mapToGlobal(button.rect().topLeft()).x() - self.tooltip_widget.width() - margin
            y = global_center.y() - self.tooltip_widget.height() // 2
            ...
            x = global_center.x() - self.tooltip_widget.width() // 2
            y = button.mapToGlobal(button.rect().topLeft()).y() - self.tooltip_widget.height() - margin
    ...
    def load_settings(self):
        ...
        self.corner_radius = int(dock_theme.get(Theme.Components.Dock.BORDER_RADIUS, 28))
        self.dock_border_width = float(dock_theme.get(Theme.Components.Dock.BORDER_WIDTH, 1.2))
        ...
        self.icon_size = min(max(self.icon_size, self.button_min_size), self.button_max_size)
        self.effective_icon_size = self.icon_size
    ...
    def available_app_ids(self):
        ...
        return [str(app.get("id", "")).strip() for app in apps if str(app.get("id", "")).strip()]
    ...
    def reload_apps(self):
        ...
        old_ids = [str(a.get("id", "")).strip() for a in (self.loaded_apps or []) if str(a.get("id", "")).strip()]
        self.ensure_default_config()
        self.load_settings()
        self.load_apps()

        new_ids = [str(a.get("id", "")).strip() for a in (self.loaded_apps or []) if str(a.get("id", "")).strip()]
        removed_ids = [rid for rid in old_ids if rid and rid not in new_ids]

        if removed_ids and hasattr(self, "runtime_service") and self.runtime_service is not None:
            for rid in removed_ids:
                try:
                    instance_ids = getattr(self.runtime_service, "instance_ids_by_app", {}).get(rid, [])
                    if instance_ids:
                        continue
    ...
    def build_buttons(self):
        ...
        self.loaded_apps = [
            app_data for app_data in self.loaded_apps
            if self.is_app_enabled(app_data)
        ]
        ...
        for button in self.buttons:
            ...
            if self.app_api is not None:
                allow_multi = self.app_api.safe_app_call(
                    ...
                    lambda: bool(instance.allow_multiple_instance()) if hasattr(instance, "allow_multiple_instance") else False,
                    default=False
                )
            else:
                try:
                    allow_multi = bool(instance.allow_multiple_instance()) if hasattr(instance, "allow_multiple_instance") else False
                except Exception:
                    allow_multi = False
    ...
    def remove_app_button(self, button, app_id):
        ...
        if app_id:
            self.loaded_apps = [
                app for app in self.loaded_apps
    ...
    def refresh_geometry_after_button_removed(self):
        ...
        length = (int(self.inner_padding) * 2) + (layout_button_count * button_size) + spacing_total
