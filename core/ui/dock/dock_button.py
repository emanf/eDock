...
class DockButton(QPushButton):
    ...
    def _app_cache_name(self):
        ...
        if isinstance(app, dict):
            return str(
                app.get("name") or app.get("title") or app.get("id") or "App"
            ).strip()
    ...
    def open_shortcut_dialog(self):
        ...
        dialog = ShortcutCaptureDialog(
            current_shortcut=self.shortcut_value, parent=self
        )
            ...
            try:
                if new_shortcut:
                    if hasattr(self.dock_window, "register_shortcut"):
                        self.dock_window.register_shortcut(
                            app_id, new_shortcut, getattr(self, "loaded_app", None)
                        )
                else:
    ...
    def remove_button(self):
        ...
        if hasattr(config_manager, "config") and isinstance(
    ...
    def _button_colors(self):
        ...
        if self.pressed:
            bg = Theme.to_ui_qcolor(
                button_theme.get(Theme.Components.Button.PRESSED_COLOR)
            )
        elif self.hovered:
            bg = Theme.to_ui_qcolor(
                button_theme.get(Theme.Components.Button.HOVER_COLOR)
            )
        else:
            bg = Theme.to_ui_qcolor(
                button_theme.get(Theme.Components.Button.BACKGROUND_COLOR)
            )

        border = Theme.to_ui_qcolor(
            button_theme.get(Theme.Components.Button.BORDER_COLOR)
        )

        border_width = float(
            button_theme.get(Theme.Components.Button.BORDER_WIDTH, 1.0)
        )
        border_radius = float(
            button_theme.get(Theme.Components.Button.BORDER_RADIUS, 18)
        )
